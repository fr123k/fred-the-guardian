...
func NewBucket(duration time.Duration) *MemoryBucket {
	bucket := MemoryBucket{
...
func NewBucketWitnCleanup(duration time.Duration) *MemoryBucket {
...
func (b *MemoryBucket) Increment(key string) Rate {
	b.mutex.Lock()
...
func (b *MemoryBucket) evicted(del uint) {
	if del > 0 && b.EvictedFunc != nil {
		b.EvictedFunc(b, del)
	}
}

func (b *MemoryBucket) randomCleanUp(maxCnt uint) uint {
	del := uint(0)
	if len(b.keys) <= 0 {
		return del
	}
...
func (b *MemoryBucket) Get(key string) *RateLimit {
	b.mutex.RLock()
	defer b.mutex.RUnlock()
	return b.counters[key]
}

func (b *MemoryBucket) Size() int32 {
	return b.size.Load().(int32)
}

// This function is not thread safe.
func (b *MemoryBucket) Print() {
